...
#####2. Create a Songs table in the above mentioned database - id, name, length, album_id
CREATE TABLE songs (
    id INT NOT NULL AUTO_INCREMENT,
    name VARCHAR(255) NOT NULL,
...
#####inserting values to the songs table
...
SELECT distinct bands.name FROM bands JOIN albums ON bands.id = albums.band_id;
    
###6. Get all the bands that have no albums
 SELECT bands.name FROM bands inner JOIN albums on bands.id = albums.band_id where bands.id != albums.band_id;   
    
###7. Get the longest album    
SELECT albums.name, COUNT(albums.name) AS count FROM albums JOIN songs ON albums.id = songs.album_id
GROUP BY albums.name
ORDER BY count DESC
LIMIT 1;
 
###8. Insert a record for your favorite Band and one of their Albums 
INSERT INTO bands(id,name) VALUES (8,'Radiohead');
INSERT INTO albums(id,name,release_year,band_id) VALUES (20,'In Rainbows',2007,8);

###9. Delete the Band and Album you added in previous question.
SET SQL_SAFE_UPDATES = 0;
delete from albums where name = 'In Rainbows';
delete from bands where name = 'Radiohead';

###10. get the Average length of all songs
select avg(length) from songs;

###11. Select the longest song of each album select 
select max(songs.length) as max_length,songs.album_id
from songs
group by album_id;

###12. Get the number of songs for each band
### Created a table called "no_of_band" joining table-bands and table-albums to get the number of albums each band has.
### Created a table called "no_of_song" joining table-songs and table-albums to get the count of songs
### Created a table called "total" to get total of songs for each album.
### songs_each_band = total_number_of_albums * sum_of_songs 
create table no_of_song(
select songs.album_id,count(songs.name) as number_of_songs,albums.band_id
from songs 
left join albums on songs.album_id = albums.id 
group by album_id);
select * from no_of_song;
create table no_of_band(
select bands.id,bands.name,count(albums.id) as number_of_albums from albums left join bands on albums.band_id = bands.id group by band_id);
select * from no_of_band;
create table total(
select sum(number_of_songs) as total,band_id from no_of_song
group by band_id);
select * from total;
create table songs_each_band(
select total.band_id,no_of_band.name,(total.total)*(no_of_band.number_of_albums) 
as songs_of_each_album from total inner join no_of_band on total.band_id = no_of_band.id);

select * from songs_each_band;
select sum(songs_of_each_album) from songs_each_band;
 
 ###13. Create a decade column with dividing the year // 10 *10.
 select min(release_year) from albums; ## To find minimun of year
 select max(release_year) from albums; ## TO find maximum of year
  
 select id,name,release_year,case
 when release_year < 1990 then "between 1980-1990"
 when release_year < 2000 then "between 1990-2000"
 when release_year < 2010 then "between 2000-2010"
 else "between 2010-2020" 
 end as "Decade"
 from albums;
 
### 14. Filter the Albums which start with the word 'The'
Select name from albums where name like "The%";

###15. Find the album which released in 2008 to 2013.
select name,release_year from albums where release_year between 2008 and 2013;
